...
Grad-CAM visualizations were generated for selected positive test examples to qualitatively inspect the image regions contributing to model predictions.
Each comparison shows, from left to right: the original image, the Grad-CAM overlay from Experiment~08 (BCE + AdamW), and the overlay from Experiment~09 (ASL + AdamW).
...
The predicted probabilities differ substantially between the two models (Exp~08: $p = 0.04$; Exp~09: $p = 0.33$), reflecting the recall-oriented behavior of ASL.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/hernia_example_01_comparison.png}
  \caption{Grad-CAM comparison for a positive Hernia test example.
    \textit{Left}: original image.
    \textit{Center}: Experiment~08 (BCE + AdamW), predicted probability 0.04.
    \textit{Right}: Experiment~09 (ASL + AdamW), predicted probability 0.33.
    Experiment~09 activates a broader and more prominent region.
    Qualitative observation only.}
  \label{fig:gradcam_hernia}
\end{figure}

Figure~\ref{fig:gradcam_mass} shows side-by-side overlays for a positive Mass test example.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/mass_example_03_comparison.png}
  \caption{Grad-CAM comparison for a positive Mass test example.
    \textit{Left}: original image.
    \textit{Center}: Experiment~08 (BCE + AdamW) overlay.
    \textit{Right}: Experiment~09 (ASL + AdamW) overlay.
    Qualitative observation only; not to be interpreted as localization.}
  \label{fig:gradcam_mass}
\end{figure}

Figure~\ref{fig:gradcam_edema} shows side-by-side overlays for a positive Edema test example.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/edema_example_02_comparison.png}
  \caption{Grad-CAM comparison for a positive Edema test example.
    \textit{Left}: original image.
    \textit{Center}: Experiment~08 (BCE + AdamW) overlay.
    \textit{Right}: Experiment~09 (ASL + AdamW) overlay.
    Qualitative observation only; not to be interpreted as localization.}
  \label{fig:gradcam_edema}
\end{figure}
